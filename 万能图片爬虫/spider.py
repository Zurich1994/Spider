...
class Task(object):
	...
	def is_finished(self):
		...
		if self.tick > 60:
			print
			"task is finished totally crawl %s urls\n" % self.current_picture_num
			return True
		return False

	def auto_regulate(self):
		print
		"current thread num %s,to_crawl %s, to_filt %s\n" % (len(threading.enumerate()), \
															 len(self.to_crawl), \
															 len(self.to_filt))
...
if __name__ == "__main__":
	...
	if results.speed_ratio == 0 or results.init_sleep_time == 0:
		print
		"speed up ratio or thread sleep time can not be zero!"
		exit(0)
	config.init(results)
	...
	while True:
		...
		if task.is_finished():
			print
			"used time %s" % (time.time() - start_time)
			break
		time.sleep(1)
